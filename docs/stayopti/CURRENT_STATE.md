...
## D-0036 browser-assisted dossier custody — implemented offline, uncommitted

- A separate evaluation-only dossier contract now represents direct manual or
  browser-assisted acquisition, independent verification states for logged-
  out/incognito/non-personalized use, multiple screenshots per alternative,
  capture-time sources, human transcription review and explicit diagnostic
  ineligibility reasons.
- The custody-v3 boundary verifies the external archive SHA-256, internal file
  checksums, safe unique paths, evidence references and timestamp ordering,
  then encrypts and reopens every original artifact before atomically exposing
  a finalized session-scoped state. Import time and collector-custody start are
  recorded separately from declared capture times. Existing custody-v2 state
  remains unchanged and readable without migration.
- Integrity-valid import never grants blind judgment, V3 replay or Golden
  admission. The preliminary browser-assisted pilot remains
  `DIAGNOSTIC_ONLY`; no real dossier was imported or accessed in this phase.
- The observed display statement `Include tasse e costi` remains distinct
  from an `UNKNOWN` itemized fiscal breakdown and does not establish an exact
  checkout price, bookability or definitive budget fit.
- Synthetic tests exercise fifteen screenshot round trips, variable evidence
  counts, failure modes, interrupted staging, overwrite refusal and v2
  compatibility. Real PowerShell 5.1/CurrentUser DPAPI validation uses only
  synthetic bytes and is not evidence of custody for a real dossier.

No network, provider call, new capture, `_002` access, public-runtime or
ranking change, real-data import, commit, push or deploy occurred.

## D-0037 relative checksums and reviewed dossier handoff — verified offline, uncommitted

- Repository observed on 2026-09-06: branch `ci/d0033-clean-commit-proof`,
  HEAD `7ae65543a8940dc2dbc93f2c72cbdd89ba8fcffe`, staged empty. D-0036
  implementation remains uncommitted; executable file hashes, not HEAD alone,
  identify the handoff. The seven original developer paths remain byte-identical.
- Checksum resolution uses exactly the containing checksum document directory.
  Legacy ZIP backslashes are normalized once; no original archive is rewritten.
- A PS5.1-only handoff separates CodeManifest, Preflight, Import and Reopen.
  Actual import requires reviewed descriptor/map/receipt, matched code/material
  hashes and a separate exact literal. Synthetic receipts cannot pass real mode.
- All originals plus four review materials are encrypted in one atomic custody
  transaction. Reopen checks exact bytes, canonical fingerprints, identities,
  counts and screenshot associations against the encrypted reviewed descriptor.
  Recomputed plaintext manifest hashes cannot conceal changed associations.
- Targeted intake/handoff: `26/26 PASS` (16 portable tests executed on Windows,
  10 Windows-only tests). Two positive lifecycles use actual PS5.1/CurrentUser
  DPAPI with synthetic evidence, including 15 genuine JPEGs named `.png`.
  This does not verify real dossier custody on the user's filesystem.
- Full canonical V3: `1545/1545 PASS`, zero skip/fail; V2: `196/196 PASS`.
  Lifecycle: `530 PASS / 17 pre-existing integration SKIP / 0 FAIL`;
  security `29/29`, release `101/101`, analytics `31/31`, capacity `9/9`,
  beta `4/4`, typecheck, build and PS5.1 parsing `3/3` PASS.
- The prior ZIP and all 45 preparation files plus their checksum manifest were
  rechecked read-only and remain unchanged. No new descriptor/review version,
  real import, real encryption or real custody session was created. Human
  review remains PENDING; the preliminary pilot remains `DIAGNOSTIC_ONLY`.
- Retention remains 14 days from custody start, with explicit purge rather
  than automatic timed erasure; reopen does not enforce expiry. Agree the
  retention/purge operation before any real import. No real authorization is
  generated by these synthetic proofs; no blind/replay/Golden bridge is added.

No network, credentials, provider call, collection, `_002` access, runtime or
ranking change, commit, push, fetch, merge or deploy occurred in this repair.

## D-0036–D-0041 software consolidation — 2026-09-08

This entry supersedes only the historical uncommitted/publication status above,
not the original evidence, review status or custody bindings. D-0038 includes
the stale-client binding and UI-continuity repair; D-0039/D-0040 provide separate
diagnostic transcription and explicit visible-group review; D-0041 prepares
diagnostic role feedback without executing V3 or granting comparison eligibility.
The complete decisions are indexed in DECISION_LOG.

The observed base is `7ae65543a8940dc2dbc93f2c72cbdd89ba8fcffe`; both actual
remote main and validation refs matched that SHA, with zero existing commits
pending publication. Consolidation targets `codex/evaluation-d0036-d0041`, not
main. The explicit candidate contains only evaluation software, synthetic tests
and operational documentation. Seven pre-existing paths are excluded, including
three private attachments and the separate real-measurement pilot changes.
All seventeen D-0037 sealed files retain their existing bytes and manifest.

The consolidation report records clean-checkout validation, exact committed
inventory, push result, remote SHA and any still-local commits. At preparation
of this entry validation/commit/push are pending; do not infer a PASS from this
document. Dependency-registry network audits are outside this Git-only network
