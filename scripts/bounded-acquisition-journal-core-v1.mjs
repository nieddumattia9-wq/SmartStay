// Shared durable one-shot journal mechanism. Endpoint capabilities are fixed by trusted profile wrappers.
...
export function createBoundedAcquisitionJournalProfile(profile){
 const LITEAPI_ACQUISITION_JOURNAL_VERSION=profile.version;
 const LITEAPI_ACQUISITION_CAPS=Object.freeze({...profile.caps});
 const kinds=Object.keys(LITEAPI_ACQUISITION_CAPS).filter(k=>!['total','concurrency','retries','redirects'].includes(k));
const repository=resolve(dirname(fileURLToPath(import.meta.url)),'..');
const fail=code=>{throw Error(profile.errorPrefix+code);};
...
const hashAcquisitionJournalValue=v=>hash(json(v));
...
function paths(input){
 ...
 if(input.mode==='REAL'){
  ...
  const expected=join(process.env.LOCALAPPDATA,'StayOpti','private-evidence',profile.registryDirectory);
...
function markerData(input){return {version:LITEAPI_ACQUISITION_JOURNAL_VERSION,caseId:input.caseId,root:input.root,
...
function keyAndHeader(input,p){const pths=paths(input),header=readJson(join(pths.root,'header.json'));
 if(!equal(header.binding,markerData(input))||!equal(header.limits,LITEAPI_ACQUISITION_CAPS)||!utc(header.startedAt))fail('CHECKPOINT_MISMATCH');
 ...
 if(!equal(Object.keys(protectedKey).sort(),['protectionClass','wrappedKey','headerSha256'].sort())||typeof protectedKey.wrappedKey!=='string'||
  protectedKey.protectionClass!==p.protectionClass||protectedKey.headerSha256!==hashAcquisitionJournalValue(header))fail('KEY_BINDING_MISMATCH');
...
function verifyWithKey(input,context){
 ...
 const journalKey=subkey(key,'journal-mac');try{
  for(let i=0;i<names.length;i++){
   ...
   if(!equal(Object.keys(value).sort(),['sequence','priorSha256','type','at','data'].sort())||value.sequence!==i+1||value.priorSha256!==prior||
    !utc(value.at)||Date.parse(value.at)<Date.parse(lastAt)||eventSha256!==hashAcquisitionJournalValue(value)||!hex(eventMac)||
 ...
 // Authenticate every event and encrypted file before any profile interpretation.
 profile.validateContext?.(input,header.context);
 if(profile.validateRequest)for(const r of summary.requests){
  const raw=decryptWithKey(input,context,r,'request');
  try{
   let request;try{request=JSON.parse(raw.toString('utf8'));}catch{fail('REQUEST_JSON_REQUIRED');}
   scanRequest(request);
   if(r.subjectSha256!==hashAcquisitionJournalValue(request.hotelId))fail('REQUEST_SUBJECT_MISMATCH');
   profile.validateRequest({input,context:header.context,request,ordinal:r.ordinal,kind:r.kind,hotelId:request.hotelId,
    snapshot:{...summary,requests:summary.requests.slice(0,r.ordinal-1),attemptsReserved:r.ordinal-1}});
  }finally{raw.fill(0);}
 }
 profile.validateSummary?.(summary,header.context);
 return {version:LITEAPI_ACQUISITION_JOURNAL_VERSION,binding:clone(header.binding),startedAt:header.startedAt,
...
function verifyAcquisitionJournal(input){const p=protectorFor(input),context=keyAndHeader(input,p);
...
function createAcquisitionJournal(input){
 ...
 profile.validateContext?.(input,input.context??null);
 ...
 const header={version:LITEAPI_ACQUISITION_JOURNAL_VERSION,binding:markerData(input),limits:LITEAPI_ACQUISITION_CAPS,startedAt,context:clone(input.context??null)};
 const append=(type,data,previous)=>{
  ...
  const eventSha256=hashAcquisitionJournalValue(value),journalKey=subkey(key,'journal-mac');
 ...
 try{
  ...
  writeJson(join(pths.root,'journal-key.json'),{protectionClass:p.protectionClass,wrappedKey,headerSha256:hashAcquisitionJournalValue(header)});
  append('START',{headerSha256:hashAcquisitionJournalValue(header)},null);
 ...
 const persistOriginal=(ordinal,direction,value)=>{
  ...
  const metadata={version:LITEAPI_ACQUISITION_JOURNAL_VERSION,caseId:input.caseId,bindingSha256:input.bindingSha256,authorizationSha256:input.authorizationSha256,
 ...
 return Object.freeze({root:pths.root,mode:input.mode,
  ...
  reserve({kind,hotelId=null,offerId=null,requestBytes,checkpointSha256}){
   ...
   const raw=bytes(requestBytes);try{let parsed;try{parsed=JSON.parse(raw.toString('utf8'));}catch{fail('REQUEST_JSON_REQUIRED');}scanRequest(parsed);
    profile.validateRequest?.({input,context:header.context,request:parsed,ordinal:s.attemptsReserved+1,kind,hotelId,snapshot:s});
   }finally{raw.fill(0);}
  ...
  finish({status}){
   ...
   profile.validateFinish?.(s,status,header.context);
