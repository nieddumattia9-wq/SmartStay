...
export function prepareDiagnosticRoleFeedback({packet,events,codeHash,reviewArchiveSha256,feedback,feedbackSha256,originalManifestSha256,budgetMinorUnits,currency}){
  ...
  // A transcription critical flag is NOT proof of rejection by an engine.
  // Preserve these findings, but separate them from demonstrated execution gates.
  const findings=missingness.filter(m=>m.critical).map(m=>({code:'MISSING_REVIEWED_CRITICAL_FIELD',alternativeId:m.alternativeId,field:m.field,reason:m.reason}));
  for(const a of state.alternatives)for(const key of ['completeTotal','ratingScale'])if(!a.fields.some(f=>f.key===key))findings.push({code:'FIELD_ABSENT_FROM_PREPARATION',alternativeId:a.id,field:key});
  return {diagnosticInput:input,diagnosticInputFingerprint:inputHash,feedbackRecord,reviewedObservations:structuredClone(state.alternatives),missingness,bindings:expected,prerequisites:{
    assessmentVersion:'stayopti.diagnostic-prerequisite-assessment@2',
    transcriptionConfirmed:true,feedbackBindingVerified:true,unknownCount:missingness.length,comparisonExecutable:false,
    // Empty means NOT ASSESSED on real canonical input, not that data passed.
    dataBlockers:[],dataBlockerAssessment:'NOT_EXECUTED_ON_CANONICAL_REAL_INPUT',transcriptionCompletenessFindings:findings,
    softwareBlockers:['REAL_REVIEW_TO_CANONICAL_V3_INPUT_NOT_IMPLEMENTED','POSTHOC_SOFT_DISTANCE_POLICY_MAPPING_NOT_VALIDATED'],
    roleCoverage:{bestChoice:'PARTIAL_COMPARISON_POSSIBLE_AFTER_INPUT_AND_CONTEXT_VALIDATION',bestOverBudget:'UNMAPPED_NOT_A_BLOCKER_FOR_BEST_CHOICE_ONLY',fullRoleComparisonExecutable:false},
    verifiedSyntheticCapabilities:{proof:'diagnosticCapabilityProbeV3 / D-0043 (D-0042 historical results preserved)',ratingScaleUnknown:'OMIT_UNNORMALIZABLE_RATING; ENGINE_CAN_COMPUTE_WITH_LOWER_EVIDENCE',completeTotalUnknown:'DISPLAY_PRICE_CAN_BE_SCORED_DIAGNOSTICALLY; INCOMPLETE_SOLUTIONS_EXCLUDED_BEFORE_SELECTION; ALL_INCOMPLETE_ABSTAINS',distance:'EXPLICIT_MAXIMUM_TRANSFERRED_BEFORE_SELECTION; maximumDistanceKm_IS_NOT_A_SOFT_PREFERENCE',realCaseExecutionInferred:false},
    existingContracts:{transcription:VERSION,prospectiveBridge:'SYNTHETIC_ONLY_ENGINE_REPLAY_DISABLED',v3Compatibility:'AdaptV2SearchResultToDecisionV3Input requires canonical V2 searchInput/result, not a review packet',externalReplay:'ExternalHotelChoiceSessionV3 is not created: no license/observational labels/complete session inferred'},researchCompleteness:'NOT_CERTIFIED',custody:'NOT_ASSESSED_NOT_REQUIRED_FOR_THIS_PREPARATION',blindEligibility:false,goldenAdmission:false,engineExecuted:false}};
}
