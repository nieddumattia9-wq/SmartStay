// D-0062: evaluation-only one-shot acquisition journal. No transport, credential
...
const kinds=['SEARCH','HOTEL_DETAIL','FACILITIES','PREBOOK','PREBOOK_GET'];
const repository=resolve(dirname(fileURLToPath(import.meta.url)),'..');
const fail=code=>{throw Error('LITEAPI_ACQUISITION_'+code);};
...
export const hashAcquisitionJournalValue=v=>hash(json(v));
...
function paths(input){
 ...
 if(input.mode==='REAL'){
  ...
  const expected=join(process.env.LOCALAPPDATA,'StayOpti','private-evidence','liteapi-controlled-acquisition');
...
function validateSummary(events,header){
 ...
 for(const e of events){const d=e.data;
  ...
  if(e.type==='RESERVE'){
   if(active!==null||!kinds.includes(d.kind)||d.ordinal!==requests.length+1||d.ordinal>17||!hex(d.subjectSha256)||seen.has(d.kind+':'+d.subjectSha256))fail('RESERVATION_INTEGRITY');
...
export function verifyAcquisitionJournal(input){const p=protectorFor(input),context=keyAndHeader(input,p);
...
export function createAcquisitionJournal(input){
 ...
 return Object.freeze({root:pths.root,mode:input.mode,
  reserve({kind,hotelId=null,offerId=null,requestBytes,checkpointSha256}){
   ...
   if(s.attemptsReserved>=17||s.counts[kind]>=LITEAPI_ACQUISITION_CAPS[kind])fail('CAP_EXCEEDED');
...
export function decryptAcquisitionOriginal(input){const p=protectorFor(input),context=keyAndHeader(input,p);
 try{const s=verifyWithKey(input,context),r=s.requests.find(r=>r.ordinal===input.ordinal),ref=r?.[input.direction];
  if(!['request','response'].includes(input.direction)||!ref)fail('ORIGINAL_NOT_FOUND');
