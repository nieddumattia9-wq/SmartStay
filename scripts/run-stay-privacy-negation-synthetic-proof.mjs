...
for(const p of ['tests/engine-v3/v3StayPrivacyNegation.test.ts','tests/engine-v3/v3StaySuitabilityContext.test.ts','package.json','package-lock.json','tsconfig.tests.json','tests/engine-v3/v3IntentPolicyRobustnessDiagnostic.test.ts','tests/engine-v3/v3IntentRolePolicyBridge.test.ts'])codePaths.push(p);
const code=codePaths.sort().map(p=>({path:p,sha256:hash(path.join(root,p))}));
const temp=fs.mkdtempSync(path.join(os.tmpdir(),'stayopti-d0047-r1-compile-')),results=[];
...
try{
  ...
  write('input-seal.json',{frozenSHA256,privacyNegationInputSHA256:hash(path.join(out,'privacy-negation-inputs.json')),codeManifestSHA256:hash(path.join(out,'code-manifest.json')),phase:'SEALED_BEFORE_EVALUATION',
