...
if($PSVersionTable.PSEdition -ne 'Desktop' -or $PSVersionTable.PSVersion.Major -ne 5 -or $PSVersionTable.PSVersion.Minor -ne 1){throw 'LITEAPI_COVERAGE_POWERSHELL_51_REQUIRED'}
$AcquisitionNode=(Get-Command node.exe -CommandType Application -ErrorAction Stop | Select-Object -First 1).Source
$AcquisitionScript=Join-Path $PSScriptRoot 'run-liteapi-search-coverage.mjs'
