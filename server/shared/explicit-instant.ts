// Canonical source. explicit-instant.mjs is its checked Node-20-compatible build.
export type ExplicitInstant = {status:'EXPLICIT_INSTANT'; original:string; epochSecond:number; fractionalSecond:string; offsetMinutes:number; reason:string} | {status:'UNINTERPRETABLE'; original:unknown; reason:string};
...
export function parseExplicitInstant(original:unknown):ExplicitInstant{
 const unknown=(reason:string):ExplicitInstant=>({status:'UNINTERPRETABLE',original,reason});
...
export function orderExplicitInstants(before:unknown,after:unknown): -1 | 0 | 1 | null {
 ...
 if(a.status!=='EXPLICIT_INSTANT'||b.status!=='EXPLICIT_INSTANT')return null;
...
export function canonicalExplicitInstant(original:unknown):string|null{
 ...
 if(parsed.status!=='EXPLICIT_INSTANT')return null;
 ...
 return new Date(parsed.epochSecond*1000).toISOString().replace(/\.000Z$/,
  (parsed.fractionalSecond?'.'+parsed.fractionalSecond:'')+'Z');
}
