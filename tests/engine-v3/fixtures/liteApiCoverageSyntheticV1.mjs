...
export function coverageFixture({count=3,directory,mutateCatalog}={}){
 ...
 const catalog={data:Array.from({length:count},(_,i)=>({id:'SYNTHETIC_OPAQUE_'+i+'+/=',name:'Invented catalog property '+i,country:'IT',city:'Bologna'}))};
 mutateCatalog?.(catalog);
 const bytes=Buffer.from(JSON.stringify(catalog));let selection=null;try{selection=selectCoverageCatalog(bytes,sha(bytes));}catch{}
 const hotel=id=>({hotelId:id,roomTypes:[{offerId:'INVENTED_OFFER_'+id,rates:[{rateId:'INVENTED_RATE_'+id,occupancyNumber:1,adultCount:2,childCount:2,childrenAges:[6,11],name:'Invented room',retailRate:{total:[{amount:777,currency:'EUR'}]}}]}]});
 const build=(kind,response)=>{const q=coverageRequest(kind,selection);return {method:q.method,path:q.path+(Object.keys(q.query).length?'?'+new URLSearchParams(q.query):''),body:q.body,response};};
