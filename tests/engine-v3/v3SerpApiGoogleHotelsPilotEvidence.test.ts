...
test("T1A 02 normalized snapshot is exported", async () => { const run = await complete(); assert.equal(run.evidence.values.size, 12); assert.ok([...run.evidence.values.values()].every((value) => value.includes(STAYOPTI_SERPAPI_PILOT_SNAPSHOT_SCHEMA_VERSION_V3))); });
...
test("T1A 04 raw is deleted after success", async () => { const run = await complete(); assert.equal(run.raw.values.size, 0); assert.equal(run.result?.rawDeletionReceipts.length, 48); });
...
test("T1A 08 property details are really merged", async () => { const run = await complete(); assert.ok(run.result?.snapshots.every((snapshot) => snapshot.alternatives.filter((entry) => entry.detailEnrichmentStatus === "MERGED").length === 3)); });
...
test("T1A 11 total request cap remains 48", async () => { const run = await complete(); assert.equal(run.calls, 48); assert.equal(run.result?.receipt.requestCount, STAYOPTI_SERPAPI_MAX_API_CALLS_V3); const ledger = new StayOptiSerpApiRequestLedgerV3(); for (let i = 0; i < 48; i += 1) { ledger.reserve("S", "PROPERTY_DETAIL"); ledger.complete(); } assert.throws(() => ledger.reserve("S", "MAIN_SEARCH"), /CAP_REACHED/); });
test("T1A 12 no retry occurs", async () => { const run = await execute({ failDetailAt: 1 }); assert.equal(run.calls, 2); });
test("T1A 13 no pagination occurs", async () => { const run = await complete(); assert.equal(run.result?.receipt.requestCount, 48); assert.equal(STAYOPTI_SERPAPI_PILOT_MANIFEST_V3.automaticPaginationAllowed, false); });
...
test("T1A 32 complete archive satisfies T3 validator", async () => { const run = await complete(); const result = validateSerpApiEvidenceArchiveEntriesV3(requiredEntries(run.result)); assert.deepEqual(result.issues, []); assert.equal(result.status, "COMPLETE"); assert.equal(result.snapshotCount, 12); assert.equal(result.replayInputAvailable, true); });
...
test("T1A 41 new authorization is generated but not granted", () => { assert.match(STAYOPTI_SERPAPI_REQUIRED_AUTHORIZATION_LITERAL_V3, new RegExp(`${STAYOPTI_SERPAPI_PILOT_MANIFEST_HASH_V3}_RUNNER_${STAYOPTI_SERPAPI_PILOT_RUNNER_BUNDLE_HASH_V3}_RETENTION_V2_MAX48$`)); const gate = readFileSync(resolve(process.cwd(), "src/engine-v3/evaluation/serpApiGoogleHotelsPilotGateV3.ts"), "utf8"); assert.match(gate, /explicitCallAuthorizationGranted:\s*false/); assert.equal(STAYOPTI_SERPAPI_PILOT_RETENTION_POLICY_VERSION_V3, "stayopti.v3.serpapi-google-hotels-retention@2"); });
