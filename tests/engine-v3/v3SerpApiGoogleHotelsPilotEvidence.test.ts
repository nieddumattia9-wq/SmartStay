...
test("T1A 04 raw is deleted after success", async () => { const run = await complete(); assert.equal(run.raw.values.size, 0); assert.equal(run.result?.rawDeletionReceipts.length, 4); });
...
test("T1A 11 historical total request cap remains 48 while canary stops at four", async () => { const run = await complete(); assert.equal(run.calls, 4); assert.equal(run.result?.receipt.requestCount, 4); assert.equal(STAYOPTI_SERPAPI_MAX_API_CALLS_V3, 48); const ledger = new StayOptiSerpApiRequestLedgerV3(); for (let i = 0; i < 48; i += 1) { ledger.reserve("S", "PROPERTY_DETAIL"); ledger.complete(); } assert.throws(() => ledger.reserve("S", "MAIN_SEARCH"), /CAP_REACHED/); });
test("T1A 12 no retry occurs", async () => { const run = await execute({ failDetailAt: 1 }); assert.equal(run.calls, 2); });
test("T1A 13 no pagination occurs", async () => { const run = await complete(); assert.equal(run.result?.receipt.requestCount, 4); assert.equal(STAYOPTI_SERPAPI_PILOT_MANIFEST_V3.automaticPaginationAllowed, false); });
...
test("T1A 41 new authorization is canary-bound and not granted", () => { assert.match(STAYOPTI_SERPAPI_REQUIRED_AUTHORIZATION_LITERAL_V3, new RegExp(`CANARY_${STAYOPTI_SERPAPI_PILOT_MANIFEST_HASH_V3}_RUNNER_${STAYOPTI_SERPAPI_PILOT_RUNNER_BUNDLE_HASH_V3}_RETENTION_V2_MAX4$`)); const gate = readFileSync(resolve(process.cwd(), "src/engine-v3/evaluation/serpApiGoogleHotelsPilotGateV3.ts"), "utf8"); assert.match(gate, /explicitCallAuthorizationGranted:\s*false/); assert.equal(STAYOPTI_SERPAPI_PILOT_RETENTION_POLICY_VERSION_V3, "stayopti.v3.serpapi-google-hotels-retention@2"); });
