...
test("T1C 23 MAX4 remains hard-bound", () => { assert.match(handoffSource, /ExpectedCanaryCap = 4/); assert.match(handoffSource, /--single-stage-max-4/); assert.doesNotMatch(handoffSource, /--single-stage-max-44/); });
...
test("T1C 30 new literal remains canary MAX4 only", () => { assert.match(STAYOPTI_SERPAPI_CANARY_AUTHORIZATION_LITERAL_V3, /^AUTHORIZE_V3_17T2_CANARY_[0-9a-f]{64}_RUNNER_[0-9a-f]{64}_RETENTION_V2_MAX4$/); assert.doesNotMatch(STAYOPTI_SERPAPI_CANARY_AUTHORIZATION_LITERAL_V3, /MAX48|REMAINING/); });
