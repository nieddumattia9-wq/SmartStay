...
export interface BoundAuthenticatedCommercialV3 {
 version:'stayopti.bound-authenticated-commercial@2';evidenceType:'authenticated-commercial-alternative-set';
...
function problems(i:Input,p:AuthenticatedCommercialPreparationV3){
 ...
 for(const o of expected){const f=o.facts.verified!,snap=i.decision.integrity.offerSnapshots.filter(x=>x.hotelId===f.identity.propertyId&&x.offerId===o.facts.decisionOfferId);
  if(snap.length!==1||snap[0].cost.total.amount===null||Math.abs(snap[0].cost.total.amount-o.qualification.verified!.cost.completeTotal!)>COMMERCIAL_EVIDENCE_DELTA_V3||snap[0].cost.total.currency!==s.currency)reasons.push('ALTERNATIVE_PRICE_OR_IDENTITY_CHANGED');
  const t=commercialFactsTermsV3(f),proof=createStayOfferIntegritySnapshotV3({hotelId:f.identity.propertyId,offerId:o.facts.decisionOfferId,
   ...
   cost:{amount:o.qualification.verified!.cost.completeTotal,currency:s.currency,completeness:'reported-complete',taxesIncluded:true,includedTaxes:0,excludedTaxes:0,unknownTaxes:0},bookable:true,recheckRequired:false,evidenceIds:[]});
 ...
 if(selected){const solutions=i.decision.solutions.filter(x=>x.kind==='single'&&x.segments.length===1&&x.segments[0].hotelId===selected.facts.verified!.identity.propertyId&&x.segments[0].offerId===selected.facts.decisionOfferId);
  if(solutions.length!==1||solutions[0].feasibility!=='feasible'||solutions[0].totalCost.completeness!=='reported-complete'||solutions[0].totalCost.currency!==s.currency||solutions[0].totalCost.amount===null||Math.abs(solutions[0].totalCost.amount-selected.qualification.verified!.cost.completeTotal!)>COMMERCIAL_EVIDENCE_DELTA_V3)reasons.push('SELECTED_SOLUTION_COST_OR_FEASIBILITY_CHANGED');
 }
 return reasons;
}
export function createBoundAuthenticatedCommercialV3(i:Input&{prepared:AuthenticatedCommercialPreparationV3}):BoundAuthenticatedCommercialV3{
 ...
 const body={version:'stayopti.bound-authenticated-commercial@2' as const,evidenceType:'authenticated-commercial-alternative-set' as const,prepared:i.prepared,
